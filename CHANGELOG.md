...
- **`pmb sync --days N` no longer stops silently at 100 commits.** `GitSync.sync` called `capture_recent_commits` without forwarding `max_commits`, so that function's default of 100 always won: `--days` moved the time window but never the cap, and the summary reported `captured: 100` with nothing to say the window held more. On a repo with 1452 commits in the last 90 days, `--days 90` captured exactly the 100 most recent and reported success. `max_commits` is now threaded CLI → `Engine.sync_git` → `GitSync.sync` → `capture_recent_commits`, with a new `--max-commits` option (`0` = no cap); the default stays 100 so existing behaviour is unchanged unless asked otherwise. `sync` additionally returns `cap_reached`, and the CLI prints a note when a run stops at the cap, so truncation is visible instead of implied. The uncapped path omits `-n` rather than passing `git log -0` (which returns nothing) and raises the subprocess timeout to 120s, since an uncapped walk over a wide window can exceed the 10s budget.
