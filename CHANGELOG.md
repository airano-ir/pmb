...
- **Captured corrections are scoped to the project they're about.** A correction-capture draft was stored as the raw user message with no project context, so it surfaced contextless and could leak across the other projects sharing a memory store. The draft now records which project the complaint is about - both as structured `project_name` metadata (so `find_lessons(project=...)` keys off it) and inline as a `[project: X]` tag - resolved via the same lenient detector the overview path uses (so dev-ish names like `pmb` are caught).
