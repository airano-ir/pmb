...
SLOS: list[SLO] = [
    ...
    SLO("hook.latency.p95", "< 300 ms thin-client overhead p95 (warm daemon)",
        "tests/hooks/test_hook_client.py::test_prepare_context_client_overhead_p95"),
    SLO("durability.backup_round_trip", "a file-copy backup restores every event",
        "tests/integration/test_backup_restore.py::test_file_copy_backup_round_trips"),
    SLO("durability.schema_self_heal", "a missing index is re-applied on Engine open",
        "tests/integration/test_backup_restore.py::test_schema_migration_self_heals_missing_index"),
